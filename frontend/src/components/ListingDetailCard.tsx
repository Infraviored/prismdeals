...
export default function ListingDetailCard({
  ...
  return (
    <Card className="p-5 justify-between hover:border-brand-accent/30 hover:-translate-y-0.5 transition-all group">
      <div className="space-y-3">
        
        {/* Image Slideshow */}
        {l.images && l.images.length > 0 && (
          <div className="relative group/gallery w-full aspect-video rounded-xl overflow-hidden bg-bg-input border border-border-subtle mb-3 shadow-md">
            <img
        ...
        {/* Card Header Info & Controls */}
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
          <div className="flex flex-col truncate pr-2">
            <span className="text-[10px] text-text-muted uppercase tracking-wider font-semibold truncate max-w-[200px]">{l.item_name}</span>
            <span className="text-[9px] text-emerald-500 font-semibold uppercase">{l.campaign_name}</span>
            {l.last_description_changed_at && (
              <span className="text-[8px] text-text-muted font-mono mt-0.5" title="Description last modified">
                {t('listing.mod', {
          ...
          <div className="flex items-center gap-1.5 flex-wrap sm:justify-end">
            {l.llm_processed_time && (
              <span className="text-[9px] text-text-muted font-medium hidden sm:inline-block">
        ...
        <div className="flex flex-wrap gap-1.5 pt-1.5">
          {l.year && (
            <span className="text-[9px] bg-bg-input text-text-muted border border-border-subtle px-2 py-0.5 rounded-md font-semibold">
          ...
          {l.cubic_capacity && (
            <span className="text-[9px] bg-bg-input text-text-muted border border-border-subtle px-2 py-0.5 rounded-md font-semibold font-mono">
              {l.cubic_capacity}
            </span>
          )}
        ...
        {/* Description Preview */}
        <div className="mt-3 text-[11px] text-text-secondary font-sans leading-relaxed border-t border-border-subtle pt-2.5">
          <p 
            style={{
              display: '-webkit-box',
      ...
      <div className="mt-4 pt-4 border-t border-border-subtle flex justify-between items-center">
        <button
          ...
          className="text-xs text-text-muted hover:text-text-primary font-bold transition-colors"
        >
      ...
      {selectedListingId === l.id && (
        <div className="mt-4 pt-4 border-t border-border-subtle space-y-4 animate-fadeIn text-xs">
          ...
          {l.llm_processed && (
            <div className="space-y-3">
                ...
                <div className="bg-rose-500/10 border border-rose-500/25 p-3.5 rounded-xl space-y-1.5 animate-fadeIn">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {l.highlights.filter(h => h.sentiment === 'negative').map((h, idx) => (
                      <span key={idx} title={`Quote: "${h.evidence_quote}"`} className="text-[10px] bg-rose-500/20 text-rose-355 px-2 py-0.5 rounded-md font-semibold border border-rose-500/20">
                        {h.label}
                      </span>
                    ))}
                  </div>
              ...
              {l.llm_processed && l.highlights && l.highlights.length > 0 && (
                <div className="bg-bg-input/60 p-3.5 rounded-xl border border-border-subtle space-y-2">
              ...
              <div className="bg-bg-input/60 p-3.5 rounded-xl border border-border-subtle">
              ...
              {/* Soft Dimensions */}
              {l.dimensions && (
                <div className="bg-bg-input/60 p-3.5 rounded-xl border border-border-subtle space-y-3">
                    ...
                    {Object.entries(l.dimensions).map(([key, dim]) => {
                      ...
                      return (
                        <div key={key} className="space-y-1">
                          ...
                          <div className="w-full bg-bg-input rounded-full h-1.5 overflow-hidden border border-border-subtle">
                            <div 
                          ...
                          {reasoning && (
                            <p className="text-[9px] text-text-muted font-sans leading-normal mt-0.5">{reasoning}</p>
                          )}
                        </div>
                      );
                    })}
              ...
              {l.reference_comparison && (
                <div className="bg-bg-input/60 p-3.5 rounded-xl border border-border-subtle space-y-2">
                  ...
                  {l.reference_comparison.reasoning && (
                    <p className="text-text-secondary font-sans text-[10px] leading-relaxed italic bg-bg-input/30 p-2.5 rounded-lg border border-border-subtle">
                      "{l.reference_comparison.reasoning}"
                    </p>
                  )}
                </div>
              ...
              <div className="bg-bg-input/60 p-3.5 rounded-xl border border-border-subtle">
                ...
                <div className="space-y-2">
                  {l.criteria_evaluations && l.criteria_evaluations.map((evalItem, idx) => (
                    <div key={idx} className="flex justify-between items-start py-1 border-b border-border-subtle last:border-0 font-sans">
                      <div className="pr-3">
                        <span className="font-bold text-text-secondary text-[11px]">{evalItem.name}</span>
                        <span className="text-[10px] text-text-muted block leading-normal mt-0.5">{evalItem.reasoning}</span>
                      </div>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${evalItem.status === 'satisfied' ? 'bg-emerald-500/10 text-emerald-400' : evalItem.status === 'violated' ? 'bg-rose-500/10 text-rose-400' : 'bg-bg-surface text-text-muted'}`}>
