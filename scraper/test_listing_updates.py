...
CREATE TABLE listings (
    ...
    images TEXT, last_seen_at TEXT, location TEXT, postal_code TEXT
);
...
def test_the_postal_code_follows_the_card_and_the_town_stays(conn):
    conn.execute("UPDATE listings SET location = 'Sendling' WHERE id = 'a'")
    changed = listing_updates.apply(
        conn,
        {"id": "a", "price_eur": 130, "location": "Anderswo", "postal_code": "81369"},
    )
    assert changed["postal_code"] == (None, "81369")
    assert conn.execute(
        "SELECT location, postal_code FROM listings WHERE id='a'"
    ).fetchone() == ("Sendling", "81369")
