...
- targets: welches Produkt gesucht wird, je Ziel "typed": Marke und Modell, bei Fahrzeugen
  auch die Generation ("Volkswagen Golf VII", "Canon EOS 5D Mark III", "Apple iPhone 13"),
  oder eine Art Ware ohne Modell ("Kinderwagen", "Waschmaschine"). Mehrere Modelle = mehrere
  Ziele. Spezifikationen gehören NICHT in das Ziel, sondern in conditions (Speicher,
  Leistung, Größe, Farbe, Laufleistung).
- conditions je Ziel, wenn sie nur dieses Ziel betreffen, sonst oben in "conditions":
  ...
  schön, negativ = stört (Kratzer -2), Stärke nach den Worten des Käufers. label ohne Einheit, deutsch, wie ein Merkmal heißt
  ("Laufleistung", "Speicher", "Gewicht"). Werte als Zahl ohne Einheit. Eine Zahl, die
  ein anderes Angebot übertreffen kann, ist eine Grenze, keine Gleichheit: wo mehr besser
  ist (Speicher, Leistung, Geschwindigkeit, Frequenz), min; wo weniger besser ist (Gewicht,
  Verbrauch, Laufleistung, Verzögerung, Alter), max. "256 GB" heißt min 256, "unter
  80000 km" max 80000. eq nur, wo ein anderer Wert nicht passt (Maß, Anschluss, Farbe). Nutze ein Merkmal der Kategorie
  nur, wenn es genau das meint und -- bei einer Optionsliste in Klammern -- der Wert eine
  dieser Optionen ist; sonst ein eigenes label (256 GB Speicher ist nicht "Speichertyp:
  SSD"). Merkmale der Kategorie: {attributes}
...
{{"category_code": "...", "name": "...", "max_price": null,
